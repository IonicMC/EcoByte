...
# YOLOv8 ONNX camera detector (primary sensor)
# Place best.onnx beside this script.
...
class ONNXBottleVerifier:
    """YOLOv8 ONNX bottle detector using OpenCV DNN.

    Primary sensor path:
      camera + best.onnx -> bottle present / not present

    If the model or camera is unavailable, the kiosk automatically falls back to
    the ultrasonic sensor so the machine still works.
    """
    def __init__(self, base_dir: str):
    ...
    def _best_conf_from_output(self, outputs) -> float:
        best = 0.0
        ...
        if arr.ndim == 2:
            rows = arr.T if arr.shape[0] < arr.shape[1] else arr
            for row in rows:
                if len(row) < 6:
                    continue
                scores = row[4:]
                if len(scores):
                    try:
                        best = max(best, float(np.max(scores)))
                    except Exception:
                        pass
            return best
    ...
    def detect_once(self, fallback_fn=None, quiet=False) -> bool:
        if not self.available or self._net is None:
            return bool(fallback_fn()) if fallback_fn is not None else False

        with self._lock:
            try:
                if not self._ensure_camera():
                    if not quiet:
                        print("[ONNX] camera failed to open; using ultrasonic fallback")
                    return bool(fallback_fn()) if fallback_fn is not None else False

                ret, frame = self._cap.read()
                if not ret or frame is None:
                    if not quiet:
                        print("[ONNX] camera frame failed; using ultrasonic fallback")
                    return bool(fallback_fn()) if fallback_fn is not None else False
                ...
                conf = self._best_conf_from_output(outputs)
                if not quiet:
                    print(f"[ONNX] confidence={conf:.3f}")
                return conf >= ONNX_CONF_THRESHOLD
            except Exception as e:
                if not quiet:
                    print(f"[ONNX] detect failed: {e}; using ultrasonic fallback")
                return bool(fallback_fn()) if fallback_fn is not None else False
...
class HardwareWorker(QThread):
    ...
    def __init__(self, presence_fn=None, verify_fn=None):
        super().__init__()
    ...
    def _sensor_present(self, quiet=True):
        if self.presence_fn is not None:
            try:
                return bool(self.presence_fn(quiet=quiet, fallback_fn=self._ultrasonic_present))
            except TypeError:
                return bool(self.presence_fn())
            except Exception as e:
                print(f"[SENSOR] presence callback error: {e}")
        return self._ultrasonic_present()
    ...
    def run(self):
        ...
        try:
            while self.running:
                ready = self._sensor_present(quiet=True)
                poll_ms = CAMERA_POLL_MS if self.presence_fn is not None else POLL_MS
                    ...
                    if not ready:
                        ...
                        if (time.monotonic() - self._verify_start) >= VERIFY_SECONDS:
                            ...
                            if self.verify_fn is not None:
                                try:
                                    allow_drop = bool(self.verify_fn(quiet=False, fallback_fn=self._ultrasonic_present))
                                except TypeError:
                                    allow_drop = bool(self.verify_fn())
                                except Exception as e:
...
class Kiosk(QStackedWidget):
    def __init__(self, verifier_fn=None):
        ...
        print(f"[ONNX] detector status: {self.verifier.reason}")
        ...
        self.worker = HardwareWorker(
            presence_fn=self.verifier.detect_once if self.verifier.enabled else None,
            verify_fn=self.verifier.detect_once if self.verifier.enabled else None
        )
